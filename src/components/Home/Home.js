...
function Home() {
  ...
  return (
    <div className={styles.parent}>
      ...
      <h1 className={styles.stepOneH}> <img className={styles.logotype} src={stepOne} /> Step One</h1>
            ...
            <div className={styles.task} key={task.id}>
              ...
              <div>
                {task?.email !== auth.currentUser?.email ? (
                  <button className={styles.buttonForOpen} onClick={() => onOpneClick(task.id)}>open</button>
                ) : null}
              </div>
            </div>
          ))}
      ...
      <div className={styles.bottom}>
        ...
        <div>
          ...
          <h5 style={{color:"#FFFACD"}}>By - [Armen Artyom Ishxan Taron]</h5>
        </div>
      </div>
    </div>
  );
